...
task("deploy", "Deploy a single contract")
  ...
  .addOptionalPositionalParam(
    "args",
    "Args of contract constructor, separated by common ','"
  )
  .setAction(async (args, hre) => {
    ...
    const linkReferences = await retrieveLinkReferences(
      "./artifacts/contracts"
    );
...
task("upgrade", "Upgrade a single contract")
  ...
  .addOptionalPositionalParam(
    "imp",
    "Address of deployed implementation, optional"
  )
  .setAction(async (args, hre) => {
...
task("send", "Send contract function transaction")
  ...
  .addOptionalPositionalParam(
    "args",
    "Args of function call, separated by common ','"
  )
  .setAction(async (args, hre) => {
    if (typeof args.args != "undefined") {
      args.args = args.args.split(",");
    ...
    await deployer.initialize();
    const callee = await deployer.getDeployedContract(
      args.contractName,
      args.aliasName
    );
