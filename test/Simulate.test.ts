...
describe("Simulate", async () => {
  ...
  it("(junior) +deposit +rebalance +withdraw", async () => {
    ...
    expect(await router.juniorLeverage(toWei("1"), toWei("0.9"))).to.equal(
      toWei("1")
    );
  ...
  it("(senior) timelock", async () => {
    ...
    await seniorConfig.setLockType(2);
    await seniorConfig.setLockPeriod(86400);
    ...
    await expect(
      router.connect(bob).withdrawSenior(toWei("200"), false)
    ).to.be.revertedWith("LOCKED");
    await expect(
      router.connect(bob).withdrawSenior(toWei("200"), true)
    ).to.be.revertedWith("LOCKED");
