...
describe("States", async () => {
  ...
  it("(junior) +deposit +rebalance +withdraw", async () => {
    /// init accounts
    const broker = await ethers.getImpersonatedSigner(
    ...
    expect(await router.pendingJuniorWithdrawal(alice.address)).to.equal(
      toWei("100")
    );
    expect(await router.totalPendingJuniorWithdrawal()).to.equal(toWei("100"));
    await time.increase(60 * 15);
    await orderBook
      .connect(broker)
      .fillLiquidityOrder(
    ...
    expect(await router.totalPendingJuniorWithdrawal()).to.equal(toWei("0"));
  });
});
